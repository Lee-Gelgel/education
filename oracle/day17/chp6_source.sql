...
INSERT INTO sign_test ( n1, n2, n3 ) 
VALUES (0.222222, 0.333333, 0.444444);

select * from sign_test;
...
select trim('abcda') trim1,
		trim(both ' ' from ' abcd ') trim2
from dual
...
SELECT NEXT_DAY(TO_DATE(sysdate), '월요일') MON,
...
to_char(sysdate,'am') ampm,
to_char(sysdate,'day') 요일
from dual;

select first_name, hire_date,to_date(sysdate,'yyyy') - to_date(hire_date,'yyyy') 근속1 ,to_char(sysdate,'yyyyddd') - to_char(hire_date,'yyyyddd') 근속2
from employees
order by hire_date

where to_char(sysdate,'yyyydd') - to_char(hire_date,'yyyydd') <= 1001
...
select job_id,nullif(job_id,'IT_PROG')
from employees
where nullif(job_id,'IT_PROG') is null
...
SELECT COALESCE('A', 'B', NULL) first,
  ...
  coalesce(to_char(manager_id),to_char(department_id),first_name)
  from employees
